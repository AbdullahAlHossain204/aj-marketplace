...
/** Used by the admin module — kept here alongside the rest of Review data
 * access rather than duplicated, the same way vendor.controller.ts reuses
 * orders.schemas directly across modules. Cross-product, any status, since
 * this is what the admin moderation queue lists from. */
export async function listReviewsForModeration(status: "PENDING" | "APPROVED" | "REJECTED" | undefined, page: number, limit: number) {
