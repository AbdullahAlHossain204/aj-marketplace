...
function shapeCart(cart: {
  ...
  items: {
    id: string;
    quantity: number;
    product: { id: string; name: string; slug: string; basePrice: number; currency: string; images: { url: string }[] };
    productVariant: { id: string; name: string; priceDelta: number; inventory: { quantity: number; reservedQuantity: number } | null };
  }[];
}) {
  const items = cart.items.map((item) => {
    const unitPrice = item.product.basePrice + item.productVariant.priceDelta;
...
const cartInclude = {
  items: {
    ...
    include: {
      product: { select: { id: true, name: true, slug: true, basePrice: true, currency: true, images: { take: 1, orderBy: { position: "asc" as const }, select: { url: true } } } },
