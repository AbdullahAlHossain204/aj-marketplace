...
async function registerAndLogin(role: "CUSTOMER" | "VENDOR" | undefined, email: string, businessName?: string) {
  await request(app)
    ...
    .send({ email, password: "Passw0rd123", name: "Test", role, businessName });
...
describe("Permission boundaries (integration)", () => {
  ...
  it("prevents Vendor B from reading Vendor A's product list via storeId tampering", async () => {
    ...
    await request(app)
      .post("/api/v1/vendor/store")
      .set("Authorization", `Bearer ${tokenA}`)
      .send({ name: `Store A ${suffix}`, slug: `store-a-${suffix}` });
  ...
  it("returns 401, not 403 or 500, for a completely unauthenticated request", async () => {
